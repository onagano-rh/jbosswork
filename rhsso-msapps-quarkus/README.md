...
`application-type=service` の設定で、このクライアントがbearer-onlyクライアントであることを伝えている。Spring Bootの用語では "Resource Server" と呼ぶ。このタイプのクライアントはアクセストークンを受け取り解釈するだけでログインに関する処理は必要ないため、設定は簡単になる。
